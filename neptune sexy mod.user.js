...
/******/ (() => {
    ...
    `);
            ...
            function initUserSelect() {
                ...
                selectField.bind("change", function () {
                    ...
                    if ($(this).val() === "__DELETE__") {
                        ...
                        const itemToDelete = unsafeWindow.prompt("Írd be a törlendő neptun kódot. Az összes törléséhez írd be: MINDEGYIKET", ["mindegyiket", ...users].join("   /   "));
                        if (!itemToDelete) {
                            return false;
                        }
                ...
                $("input[type=button].login_button")
                    ...
                    .bind("click", function (e) {
                        ...
                        if ($("#pwd").val() !== atob(storage.get("users", utils.getDomain(), users[$("#user_sel").get(0).selectedIndex], "password"))) {
                            if (confirm(`Szeretnéd megváltoztatni a(z) ${$("#user").val().toUpperCase()} felhasználó tárolt jelszavát a most beírt jelszóra?`)) {
                                storage.set("users", utils.getDomain(), users[$("#user_sel").get(0).selectedIndex], "password", btoa($("#pwd").val()));
                            }
                        }
        ...
        /***/ 977: /***/ (module, __unused_webpack_exports, __webpack_require__) => {
            const $ = window.jQuery;
            const utils = __webpack_require__(555);
    ...
    `);
        ...
        /***/ 104: /***/ (module, __unused_webpack_exports, __webpack_require__) => {
            const $ = window.jQuery;
            const utils = __webpack_require__(555);
            const storage = __webpack_require__(75);
                ...
                window.setInterval(() => {
                    ...
                    if (table.size() > 0 && table.attr("data-choices-displayed") !== "1") {
                        ...
                        if ($("#npu_clear_courses").size() === 0) {
                            const clearAll = $('<td id="npu_clear_courses"><a style="color: #C00; line-height: 17px; margin-right: 30px" href="">Tárolt kurzusok törlése</a></td>');
                            $("a", clearAll).click(function (e) {
                                e.preventDefault();
                                if (confirm(`${utils.getNeptunCode()} felhasználó összes tárolt kurzusa törölve lesz ezen a képzésen. Valóban ezt szeretnéd?`)) {
                    ...
                    const innerTable = $("#Addsubject_course1_gridCourses_bodytable:visible, #Addsubject_course1_gridCourses_grid_body_div:visible").first();
                        ...
                        if (subjectText !== null) {
                            ...
                            if (subjectCode) {
                                ...
                                if ($(".npu_course_choice_actions").size() === 0) {
                                    ...
                                    if (header.size() === 0) {
                                        $(`<table class="grid_functiontable_top" align="left"><tbody><tr>` + `<td class="functionitem" nowrap=""></td>` + `</tr></tbody></table>`).appendTo(
                                            "#Addsubject_course1_gridCourses_gridmaindiv .grid_topfunctionpanel"
                                        );
                                        header = $(header.selector);
                                    }
                                    if (footer.size() === 0) {
                                        $(`<table class="grid_functiontable_bottom" align="right"><tbody><tr>` + `<td class="functionitem" nowrap=""></td>` + `</tr></tbody></table>`).appendTo(
                                            "#Addsubject_course1_gridCourses_tablebottom .grid_bottomfunctionpanel"
                                        );
                                        footer = $(footer.selector);
                                    }
                                    const loadAndSaveHtml = canSave ? '<input type="button" value="Betöltés és Mentés" class="gridbutton npu_course_choice_apply" style="display: none">' : "";
    ...
    `);
                ...
                $(
                    `<span id="npuStatus" style="font-weight: normal">` + `<a href="https://github.com/solymosi/npu" target="_blank">Neptun PowerUp!</a> ` + `v${GM.info.script.version}` + `</span>`
            ...
            function fixOfficialMessagePopup() {
                ...
                window.setInterval(() => {
                    ...
                    if (popup.size() > 0 && popup.is(":not([data-npu-enhanced])")) {
                        ...
                        const dismissBtn = $('<input value="Most nem érdekel" class="npu_dismiss ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" type="button">');
                ...
                window.setInterval(() => {
                    ...
                    if (table.attr("data-processed") !== "1") {
                        ...
                        $("tbody tr:not(.NoMatch)", table).each(function () {
                            ...
                            const attended = $("td[n=Attended]", row)[0].attributes.checked.value === "true" || $("td[n=JustifiedMissing]", row)[0].attributes.checked.value === "true";
    ...
    `);
                const findTermSelect = () => {
                    return $(
                        ["#upFilter_cmbTerms", "#upFilter_cmb_m_cmb", "#cmbTermsNormal", "#upFilter_cmbTerms_m_cmb", "#cmb_cmb", "#c_common_timetable_cmbTermsNormal", "#cmbTerms_cmb"].join(", ")
                    ).first();
                };
                const blanklist = ["0303", "h_addsubjects", "0401", "h_exams", "0503", "h_transactionlist", "1406", "h_grading_request"];
        ...
        /***/ 555: /***/ (module) => {
            ...
            function isPassingGrade(str) {
                return ["jeles", "excellent", "jó", "good", "közepes", "satisfactory", "elégséges", "pass", "kiválóan megfelelt", "excellent", "megfelelt", "average"].some(function (item) {
            ...
            function isFailingGrade(str) {
                return ["elégtelen", "fail", "nem felelt meg", "unsatisfactory", "nem jelent meg", "did not attend", "nem vizsgázott", "did not attend"].some(function (item) {
                    return str.toLowerCase().indexOf(item) !== -1;
                });
            }
...
function SetBaseColor() {
    let c = HexToHSL(localStorage.getItem("base-color"));
